...
public class MultiScreenServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (create != null) {
			...
			CreateGroupStream(out, matrix, rows, columns);
	...
	protected void CreateGroupStream(PrintWriter out, String path, String rows, String columns) {
		int y = Integer.parseInt(rows);
		int x = Integer.parseInt(columns);
		StreamGroup sGroup = new StreamGroup(path, y, x);
		groups.add(sGroup);
		ProcessBuilder pb = new ProcessBuilder("videospliter.sh", "-i", path, "-r", rows, "-c", columns);
	...
	protected void VideoPage(PrintWriter out, String path, String selectedPosition, HttpServletRequest request) {
		...
		temp = path.split("\\.");
		String videoLink = ffServerUrl + temp[0] + "_" + x + "_" + y + "." + temp[1];
		out.println("<div><video width='1280' height='720' controls> <source src='" + videoLink + "'"
	...
	protected void ShowFilesList(PrintWriter out, HttpServletRequest request) {
		String path = request.getSession().getServletContext().getRealPath(videosPath);
		...
		if (folder.exists()) {
			...
			out.println("PLEASE MAKE VIDEOS FOLDER IN: " + path);
		}
